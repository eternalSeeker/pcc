...
class CompareNotEqual(BinaryOperator):
    def __init__(self, depth, operand_1, operand_2):
        super(CompareNotEqual, self).__init__(depth, operand_1, operand_2)
        self.operator = '!='
    ...
    def evaluate(self, source, destination, assembler):
        ...
        jump_over_set_result = assembler.je(jump_amount)

        # compare the 2 value, clean the destination and if the 2 values
        # were not equal, set the result, else jump over these instructions
        value += cmp_values
        value += clear_result
