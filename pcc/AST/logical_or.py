...
class LogicalOr(BinaryOperator):
    def __init__(self, depth, operand_1, operand_2):
        super(LogicalOr, self).__init__(depth, operand_1, operand_2)
        self.operator = '||'
    ...
    def evaluate(self, source, destination, assembler):
        ...
        """
        value = bytearray()
        value += assembler.logical_or(source, destination)
        return value
    ...
    def load_result_to_reg(self, register, assembler):
        ...
        jump_amount = len(else_instructions) + len(clear_result)
