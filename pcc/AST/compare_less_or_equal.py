...
class CompareLessOrEqual(BinaryOperator):
    def __init__(self, depth, operand_1, operand_2):
        super(CompareLessOrEqual, self).__init__(depth, operand_1, operand_2)
        self.operator = '<='
    ...
    def evaluate(self, source, destination, assembler):
        ...
        jump = assembler.jg(jump_amount)

        # compare the 2 value, clear the destination and if the destination
        # is less or equal then source, set the result, else jump over these
        # instructions
        value += compare
        value += clear_result
        value += jump
